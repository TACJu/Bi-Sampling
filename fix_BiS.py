...
parser = argparse.ArgumentParser(description='PyTorch FixMatch BiS Training')
...
# Sampler options
parser.add_argument('--sampler1', default='random', type=str, choices=['random', 'mean', 'reverse'],
                    help='First sampler for labeled data')
parser.add_argument('--semi-sampler1', default='random', type=str, choices=['random', 'mean', 'reverse'],
                    help='First sampler for unlabeled data')
parser.add_argument('--sampler2', default='mean', type=str, choices=['random', 'mean', 'reverse'],
                    help='Second sampler for labeled data')                    
parser.add_argument('--semi-sampler2', default='mean', type=str, choices=['random', 'mean', 'reverse'],
                    help='Second sampler for unlabeled data')
...
def main():
    ...
    if args.sampler1 == 'random':
        labeled_trainloader1 = data.DataLoader(train_labeled_set, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
    elif args.sampler1 == 'mean':
        labeled_trainloader1 = data.DataLoader(train_labeled_set, batch_size=args.batch_size, sampler=mean_sampler, num_workers=4, drop_last=True)
    elif args.sampler1 == 'reverse':
        labeled_trainloader1 = data.DataLoader(train_labeled_set, batch_size=args.batch_size, sampler=reverse_sampler, num_workers=4, drop_last=True)

    if args.sampler2 == 'random':
        labeled_trainloader2 = data.DataLoader(train_labeled_set, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
    elif args.sampler2 == 'mean':
        labeled_trainloader2 = data.DataLoader(train_labeled_set, batch_size=args.batch_size, sampler=mean_sampler, num_workers=4, drop_last=True)
    elif args.sampler2 == 'reverse':
        labeled_trainloader2 = data.DataLoader(train_labeled_set, batch_size=args.batch_size, sampler=reverse_sampler, num_workers=4, drop_last=True)
    ...
    if args.semi_sampler1 == 'random':
        desi_p1 = desi_p_o
    elif args.semi_sampler1 == 'mean':
        desi_p1 = desi_p_m
    elif args.semi_sampler1 == 'reverse':
        desi_p1 = desi_p_r

    if args.semi_sampler2 == 'random':
        desi_p2 = desi_p_o
    elif args.semi_sampler2 == 'mean':
        desi_p2 = desi_p_m
    elif args.semi_sampler2 == 'reverse':
        desi_p2 = desi_p_r
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Training part
        train_loss, train_loss_x, train_loss_u = train(labeled_trainloader1, labeled_trainloader2, unlabeled_trainloader, model, optimizer,
                                                       ema_optimizer, train_criterion, epoch, use_cuda, desi_p, alpha)
...
def train(labeled_trainloader1, labeled_trainloader2, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda,
          desi_p, alpha):
    ...
    labeled_train_iter1 = iter(labeled_trainloader1)
    labeled_train_iter2 = iter(labeled_trainloader2)
    unlabeled_train_iter = iter(unlabeled_trainloader)

    model.train()
    for batch_idx in range(args.val_iteration):
        try:
            inputs_x1, targets_x1, _ = labeled_train_iter1.next()
            inputs_x2, targets_x2, _ = labeled_train_iter2.next()
        except:
            labeled_train_iter1 = iter(labeled_trainloader1)
            labeled_train_iter2 = iter(labeled_trainloader2)
            inputs_x1, targets_x1, _ = labeled_train_iter1.next()
        ...
        inputs_x = torch.cat((inputs_x1[index1], inputs_x2[index2]), dim=0) 
        targets_x = torch.cat((targets_x1[index1], targets_x2[index2]), dim=0)
